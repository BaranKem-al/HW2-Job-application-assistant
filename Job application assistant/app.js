...
// ============================================================
// AGENT 3: PUBLISHER
// Saves cover letter to file
// ============================================================
function agent3_publisher(coverLetter, company, position) {
...
// Logs application data to Google Sheets (HW2 requirement)
// ============================================================
async function agent4_sheetsLogger(name, email, parsedData, fileInfo) {
  ...
  try {
    ...
    const row = [
      name,                        // Name
      email,                       // Email
      parsedData.position,         // Position Applied
      parsedData.company,          // Company
      parsedData.skills.join(', '), // Skills
      'Cover Letter Generated',    // Status
      today,                       // Date
      fileInfo.fileName            // Output File
    ];
...
app.post('/apply', async (req, res) => {
  ...
  try {
    ...
    // AGENT 3: Save to file
    const fileInfo = agent3_publisher(coverLetter, parsedData.company, parsedData.position);
    ...
    res.json({
      ...
      pipeline: {
        ...
        agent3_publisher: { status: 'SUCCESS', file: fileInfo.fileName },
        agent4_sheets: sheetsResult.success ? { status: 'SUCCESS' } : { status: 'ERROR', error: sheetsResult.error }
      },
      cover_letter: coverLetter,
      sheets_logged: sheetsResult.success,
      email_confirmation: `Cover letter has been sent to ${email}`
    });
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      'POST /apply': 'Submit job description → get cover letter + Google Sheets log',
    },
    payload_example: {
      name: 'Baran Yazici',
